...
* DB インスタンスタイプの用語変更
    ...
    * フェイルオーバーされたマスター → Failed Over Primary
...
* 断続的にネットワーク通信の失敗により、高可用性 DB が起動しない場合があるバグを修正しました。
* DB ポートの変更に失敗した際、断続的にガベージセキュリティグループルールが残存するバグを修正しました。
...
* MariaDB 10.11.18、11.4.12、11.8.8バージョンを追加
...
* セキュリティパッチ機能を追加
    ...
    * 詳細については、[セキュリティパッチ](/Database/RDS%20for%20MariaDB/ja/db-instance/#security-patch)のドキュメントを参照してください。
* 高可用性Pingチェック方式に、従来の`INSERT`に加えて`SELECT`オプションを追加
* MariaDB 10.6.25, 10.11.16, 11.4.10, 11.8.6バージョン追加
* MariaDB 10.3、10.6のEOL(End of Life)に伴う新規作成の制限
* API v4.0の追加及び変更
    * 高可用性情報の照会APIが追加されます。
...
* API v4.0の追加
    * 詳細については、[API v4.0 ガイド](/Database/RDS%20for%20MariaDB/ja/api-guide-v4.0/)ドキュメントを参照してください。
* スナップショットバックアップ機能の追加
...
* MariaDB 10.6.22, 10.11.13, 11.4.7バージョン追加
...
* ユーザーグループにユーザーを追加する際、組織、プロジェクト内のすべてのユーザーを追加できるように改善
...
* MariaDBに登録されたユーザーが多すぎる場合、復元に失敗する現象を修正
...
* コンソールでDBスキーマを作成する時、入力可能な名前の最小文字数をMariaDBと同じに修正

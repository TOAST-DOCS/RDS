...
| リージョン | エンドポイント |
|------|----------|
| 韓国(テグ)リージョン | https://ngoic-kr4-rds-proxy.cloud.toastoven.net |


<a id="common-authorization"></a>
### 認証および権限 { #common-authorization }

RDS for MySQL APIを使用するには、User Access Keyが必要です。User Access Keyは、NHN CloudアカウントまたはIAMアカウントに基づいて発行される認証キーであり、Secret Access Keyと共に使用してAPIリクエストの認証手段として利用されます。

User Access KeyとSecret Access Keyは、コンソールの**APIセキュリティ設定**で発行できます。User Access Keyの発行及び使用方法は[User Access Key](/nhncloud/ja/public-api/user-access-key)を参照してください。
作成されたKeyはAppkeyと一緒にリクエストヘッダに含める必要があります。
...
### DBエンジンバージョンリストを表示 { #get-db-versions }
...
### DBインスタンス仕様リストを表示 { #get-db-flavors }
...
### DBインスタンスグループの詳細を表示 { #get-db-instance-group }
...
### オブジェクトストレージから復元 { #restore-db-instance-from-obs }
...
### DBインスタンスの詳細を表示 { #get-db-instance }
...
### DBインスタンスを修正する { #update-db-instance }
...
### バックアップ情報を修正する { #update-backup-info }
...
### テスト用DBイメージメタの変更 { #change-image-meta }
...
### DBインスタンス削除保護設定を変更する { #change-deletion-protection }
...
### 高可用性を修正する { #update-high-availability }
...
### 高可用性を復旧する { #repair-high-availability }
...
### 高可用性を再開する { #resume-high-availability }
...
### 高可用性を分離する { #split-high-availability }
...
### ログファイルのエクスポート { #export-log-files }
...
### ネットワーク情報を修正する { #update-network-info }
...
### 復元される最後のクエリ照会 { #get-last-query-to-restore }
...
### ストレージ情報を修正する { #update-storage-info }
...
### DBセキュリティグループの詳細を表示 { #get-db-security-group }
...
| dbSecurityGroup.progressStatus | Enum | DBセキュリティグループの現在進行状態<br/>- `NONE`: なし<br/>- `CREATING_RULE`: ルール作成中<br/>- `UPDATING_RULE`: ルール修正中<br/>- `DELETING_RULE`: ルール削除中<br/>- `APPLYING_DEFAULT_RULE`: 基本ルール適用中 |
...
| dbSecurityGroup.rules.port.portType | Enum | ポートタイプ<br/>- `ALL`: ポート範囲全体(ユーザーコンソールでは使用しません)<br/>- `PORT`: 特定ポート<br/>- `DB_PORT`: DB受信ポート<br/>- `PORT_RANGE`: ポート範囲 |
...
### DBセキュリティグループを修正する { #update-db-security-group }
...
### DBセキュリティグループルールを修正する { #update-db-security-group-rule }
...
### パラメータグループの詳細を表示 { #get-parameter-group }
...
### パラメータを修正する { #modify-parameter-group-parameters }
...
| selectAllYN | Boolean | N | プロジェクトメンバー全体を含めるかどうか<br/>- デフォルト値: `false` |
...
### ユーザーグループを修正する { #update-user-group }
...
### 通知グループの詳細を表示 { #get-notification-group }
...
### 通知グループを修正する { #update-notification-group }
...
### 購読可能なイベントコード一覧表示 { #get-event-codes }
...
### イベント購読一覧照会 { #get-event-subscriptions }
...
### イベント購読作成 { #register-event-subscription }
...
### イベント購読削除 { #delete-event-subscription }
...
### イベント購読修正 { #modify-event-subscription }

...
RDS for {{engine.pascalCase}}は、DBインスタンスにインストールされた{{engine.pascalCase}}の設定を適用するために、パラメータグループ機能を提供します。パラメータグループは、{{engine.pascalCase}}を設定できるパラメータの集合です。サービスの有効化時に、すべてのDBエンジンのバージョンごとに基本パラメータグループを提供します。基本パラメータグループは `default.{DBエンジンバージョン名}` として提供され、バージョンごとに推奨される基本パラメータ値で構成されています。DBエンジンバージョンファミリーごとの基本パラメータグループも合わせて提供されます。基本パラメータグループは、通常のパラメータグループと同様に変更または削除できます。
...
!!! tip "参考"
    [マイナーバージョン自動アップグレード](db-instance/#maintenance-auto-minor-version-upgrade)は、ファミリーパラメータグループを使用するDBインスタンスにのみ適用されます。

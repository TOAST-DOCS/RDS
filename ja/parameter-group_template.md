...
RDS for {{engine.pascalCase}} は、DB インスタンスにインストールされた {{engine.pascalCase}} の設定を適用するために、パラメータグループ機能を提供します。パラメータグループは、{{engine.pascalCase}} を設定できるパラメータの集合です。サービスの有効化時に、すべての DB エンジンのバージョンごとにデフォルトパラメータグループを提供します。デフォルトパラメータグループは `default.{DBエンジンバージョン名}` として提供され、バージョンごとに推奨されるデフォルトパラメータ値で構成されています。DB エンジンバージョンファミリーごとのデフォルトパラメータグループも合わせて提供されます。デフォルトパラメータグループは、通常のパラメータグループと同様に変更または削除できます。
...
| タイプ      | 適用範囲                                | デフォルトパラメータグループ名            |
|---------|--------------------------------------|-------------------------|
| ファミリー（推奨） | 同じメジャーバージョンに属するすべてのマイナーバージョンのDBインスタンス     | `default.{DBエンジンバージョンファミリー}` |
| シングル      | 指定したDBエンジンバージョン1つを使用するDBインスタンス        | `default.{DBエンジンバージョン名}`  |

DBエンジンバージョンファミリーは、メジャーバージョンと1対1で対応するDBエンジンバージョンの集合です。メジャーバージョンが同じマイナーバージョンは、すべて1つのDBエンジンバージョンファミリーに属します。

ファミリーパラメータグループを使用すると、マイナーバージョンが異なるDBインスタンスに同じパラメータグループをまとめて適用できます。マイナーバージョンごとにパラメータグループを個別に作成・管理する必要がありません。詳細については、[ファミリーパラメータグループ](parameter-group/#family-parameter-group)を参照してください。

<a id="create-parameter-group"></a>
### パラメータグループの作成 { #create-parameter-group }

必要に応じて、パラメータコンソールでパラメータグループを作成できます。パラメータグループを作成する際は、まずパラメータグループタイプを選択します。ファミリーを選択した場合は DB エンジンバージョンファミリーを、単一を選択した場合は DB エンジンバージョンを指定します。作成後はタイプを変更することはできません。
...
DBインスタンスの作成または変更時に、DBインスタンスに適用するパラメータグループを選択できます。1つのDBインスタンスには1つのパラメータグループが適用され、1つのパラメータグループは複数のDBインスタンスに適用できます。選択リストには、DBインスタンスのDBエンジンバージョンと同じシングルパラメータグループと、そのバージョンが属するDBエンジンバージョンファミリーのファミリーパラメータグループが併せて表示されます。パラメータグループのパラメータが変更された場合、その変更内容はすぐにDBインスタンスに適用されません。関連するDBインスタンスが存在する場合、パラメータグループは `適用必要` 状態に変更されます。DBインスタンス一覧画面でパラメータグループと関連付けられたDBインスタンスを選択し、**[パラメータグループ変更の適用]** をクリックすることで、パラメータの変更内容をDBインスタンスに反映できます。関連するすべてのDBインスタンスにパラメータグループの変更内容が適用されると、パラメータグループは `適用完了` 状態に変更されます。
...
ファミリーパラメータグループは、同じ DB エンジンバージョンのファミリー、つまり同じメジャーバージョンに属するすべてのマイナーバージョンの DB インスタンスに共通して適用できるパラメータグループです。同じメジャーバージョンを使用している場合、マイナーバージョンが異なっていても 1 つのパラメータグループで管理でき、マイナーバージョンをアップグレードしてもパラメータグループを再選択する必要はありません。

!!! tip "ヒント"
    [マイナーバージョンの自動アップグレード](db-instance/#maintenance-auto-minor-version-upgrade)は、ファミリーパラメータグループを使用する DB インスタンスにのみ適用されます。
...
DB インスタンスには、自身の DB エンジンバージョンが属する区間の値が適用されます。
...
### デフォルトファミリーパラメータグループ { #default-family-parameter-group }

サービスを有効にすると、DBエンジンバージョンファミリーごとにデフォルトファミリーパラメータグループが `default.{DBエンジンバージョンファミリー}` という名前で提供されます。すべてのパラメータが各マイナーバージョンのデフォルト値で構成されており、通常のファミリーパラメータグループと同様に変更または削除できます。
...
| テンプレート区間 | 値が適用される DB エンジンバージョンの範囲です。<br/>ファミリーパラメータグループにのみ表示されます。 |

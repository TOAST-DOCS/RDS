...
- DBセキュリティグループの送信ルールでDBポートタイプを指定できないように改善
- 高可用性DBインスタンスの予備マスター名も入力できるように修正
- Resource WatcherでDBインスタンスのリソースを確認できるように改善
- フェイルオーバーが発生したマスターも削除するまで正常に課金するように修正
- バイナリログがないためフェイルオーバーが発生したマスターを復旧できない場合、正確なエラーメッセージを表示するように改善

#### バグ修正

- バックアップのエクスポート時にパスに特殊文字が含まれるとバックアップが失敗する問題を修正
- ユーザーグループを削除する際、イベント購読から該当ユーザーグループが削除されない問題を修正
- 重複した通知グループを削除する際、正確なエラーメッセージを表示するように改善

...
障害発生時に備えて、DB インスタンスのデータベースを復旧できるよう、事前に準備できます。必要なときにコンソールからバックアップを実行したり、定期的にバックアップが実行されるよう設定したりできます。バックアップの実行中は、対象 DB インスタンスのストレージのパフォーマンスが低下する場合があります。サービスへの影響を避けるため、サービスの負荷が少ない時間帯にバックアップを実行することをお勧めします。バックアップによるパフォーマンス低下を避けたい場合は、高可用性構成を使用するか、直前のバックアップ以降のデータの増分のみをバックアップするか、Read Replica でバックアップを実行することもできます。

!!! tip "ヒント"
    高可用性 DB インスタンスは Standby でバックアップが実行されるため、Primary のストレージパフォーマンスの低下は発生しません。
    ただし、次の場合は高可用性 DB インスタンスであっても Primary でバックアップが実行される場合があります。
    * Standby の障害によりバックアップの実行が不可能な状態の場合
    * Standby の再構築に Standby 以外の DB インスタンスで実行したバックアップが必要であるが、Read Replica がない場合
...
!!! tip "ヒント"
    2023年8月17日に XtraBackup ユーティリティのバージョンがアップグレードされました。以前のバックアップに使用された XtraBackup のバージョンはコンソールで確認できます。
{{/if}}
...
!!! danger "注意"
    増分バックアップから復元する場合は、全体バックアップから復元する場合よりも多くの時間がかかる場合があります。これは、復元に必要な増分バックアップの容量の合計に比例します。
...
[自動バックアップ戦略](#set-auto-backup)に従って増分バックアップが実行される場合、上記の制約事項に加えて、次の追加制約事項を満たすベースラインバックアップが自動的に選択されます。制約事項を満たすベースラインバックアップが存在しない場合は、自動バックアップ戦略に関係なく全体バックアップを実行します。
* レプリケーション中断状態の Standby、Read Replica で実行されたバックアップはベースラインバックアップになることができません。
* テーブルロックを使用しない状態で実行されたバックアップはベースラインバックアップになることができません。
* 該当バックアップの作成以降に新しい全体バックアップが作成された場合、ベースラインバックアップになることができません。
...
!!! danger "注意"
    増分方式で作成されたバックアップは、自動バックアップの保管期間が経過していない場合でも、ベースラインバックアップが削除されると同時に削除されます。
...
!!! danger "注意"
    前のバックアップが終了していない場合など、バックアップが実行されないことがあります。
    スケジュール上、増分バックアップを実行する順番であっても、増分可能なベースラインバックアップが存在しない場合は、全体バックアップが実行されることがあります。
    増分可能なベースラインバックアップの詳細については、[ベースラインバックアップ](#baseline-backup)を参照してください。
...
バックアップ後、バックアップファイルをオブジェクトストレージにエクスポートできます。増分バックアップについてはサポートされていません。
...
内部バックアップストレージに保存されたバックアップファイルをオブジェクトストレージにエクスポートできます。増分バックアップについてはサポートされていません。
...
!!! tip "ヒント"
    手動バックアップの場合、バックアップを実行した元の DB インスタンスが削除されている場合は、バックアップをエクスポートできません。
...
!!! danger "注意"
    復元先の DB インスタンスのデータストレージサイズが、バックアップを実行した元の DB インスタンスのデータストレージサイズより小さい場合、または元の DB インスタンスのパラメータグループと異なるパラメータグループを使用している場合、復元に失敗する可能性があります。
...
!!! danger "注意"
    innodb_data_file_path の設定値が ibdata1:12M:autoextend でない場合、RDS for {{engine.pascalCase}} の DB インスタンスに復元することはできません。
...
```
mariabackup --defaults-file={my.cnf のパス} --user {ユーザー} --password '{パスワード}' --socket {MariaDB ソケットファイルのパス} --compress --compress-threads=1 --stream=xbstream {バックアップファイルが生成されるディレクトリ} 2>>{バックアップログファイルのパス} > {バックアップファイルのパス}
```
{{/if}}

(2) バックアップログファイルの最終行に `completed OK!` があるかどうかを確認します。`completed OK!` がない場合は、バックアップが正常に終了していないため、ログファイルに記載されているエラーメッセージを参照してバックアップを再実行します。
...
{{#if (eq engine.lowerCase "mysql")}}
!!! danger "注意"
    現在、バージョン 5.7.33 では、オブジェクトストレージのバックアップファイルを使用した DB インスタンスの復元は制限されています。
    推奨する XtraBackup 以外のバージョンを使用すると、正常に動作しない場合があります。
    オブジェクトストレージのバックアップファイルと復元先の MySQL のバージョンは同一である必要があります。
...
(1) [バックアップのエクスポート](backup-and-restore/#export)の項目を参照して、RDS for {{engine.pascalCase}}のバックアップをオブジェクトストレージにエクスポートします。

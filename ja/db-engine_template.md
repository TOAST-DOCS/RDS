...
MySQL 8.0.34 未満のバージョンは、MySQL LTS サポートポリシーに従いサポートが終了しました。該当バージョンの DB インスタンスは、最新バージョンへアップグレードすることをお勧めします。
...
| MySQL 8.0.35         | 新規に作成したり、Read Replica を追加したりすることはできません。  |
| MySQL 8.0.34         | 新規作成またはRead Replicaの追加はできません。  |
| MySQL 8.0.33         | 新規に作成したり、Read Replica を追加したりすることはできません。  |
| MySQL 8.0.32         | 新規に作成したり、Read Replica を追加したりすることはできません。  |
| MySQL 8.0.28         | 新規に作成することも、Read Replica を追加することもできません。  |
| MySQL 8.0.23         | 新規に作成したり、Read Replica を追加したりすることはできません。  |
...
!!! danger "注意"
    DB バージョン変更時はアップグレードのみサポートしており、ダウングレードはサポートしていません。
...
DB インスタンス修正画面で DB エンジンバージョンの変更を試みる際に、ダミー DB インスタンスの使用有無を選択することで、バージョンアップグレード中も高可用性を維持できます。ダミー DB インスタンスの使用を選択すると、DB バージョンアップグレード用の Standby が作成されます。

!!! danger "注意"
    ダミー DB インスタンスはアップグレード中に一時的な Standby を作成するため、このオプションは高可用性構成でない場合にのみ使用できます。
...
DB インスタンスが高可用性で構成されている場合、Standby のエンジンバージョンを先にアップグレードした後、フェイルオーバーを使用して Standby を Primary に切り替えます。フェイルオーバーは Primary のサービスを一時的に中断するため、ユーザーが任意のタイミングでフェイルオーバーを開始できます。
バージョンアップグレード時にフェイルオーバー手動制御設定を使用すると、ユーザーがコンソールから直接フェイルオーバーを開始できます。

!!! danger "注意"
    フェイルオーバーの手動制御が 60 時間以上トリガーされない場合、アップグレード作業は自動的にキャンセルされます。

<a id="when-using-an-outdated-operating-system"></a>
### 古いOSを使用する場合 { #when-using-an-outdated-operating-system }

内部オペレーティングシステムが古い DB インスタンスは、DB バージョンをアップグレードする前に、VM の交換を伴うオペレーティングシステムバージョンのアップグレードが必要です。通知グループの監視対象インスタンスおよびイベントサブスクリプションのイベントソースは、変更された識別子に自動的に置き換えられます。単一 DB インスタンスは、DB バージョン変更時にダミー DB インスタンスの使用をお勧めします。高可用性 DB インスタンスは、DB インスタンスの交換過程でフェイルオーバーを利用して Primary と Standby の役割が変更されます。Primary の負荷が高い場合、フェイルオーバーに失敗する可能性があるため、DB バージョンの変更は負荷が高くない時間帯に実施することをお勧めします。

!!! danger "注意"
    内部 IP を IP ACL またはセキュリティグループで直接使用している既存の DB インスタンスがある場合は、注意が必要です。
...
!!! danger "注意"
    MySQL のバージョンによっては対応していない場合があります。非対応のバージョンにアップグレードすると、該当のプラグインを使用することはできません。

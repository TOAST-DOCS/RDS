...
> [参考]
> 2025年7月のメンテナンス以降、高可用性DBインスタンスについては、マスターだけでなく予備マスターの名前も入力するように変更されました。予備マスターの名前もマスターと同じ制約条件を持ち、マスターと予備マスターの名前は異なっている必要があります。メンテナンス以前に作成されたDBインスタンスの場合、予備マスターの名前はマスターと同じです。
...
2025年5月のメンテナンス以降に作成されたDBインスタンスは、VIP（Virtual IP）をサポートします。VIPはユーザーのVPCサブネットに属するIPアドレスを指します。高可用性DBインスタンスの場合、VIPは常に現時点のマスターを指します。アプリケーションの接続情報には、必ずVIPを直接使用するか、VIPを指す内部（VIP）ドメインを使用する必要があります。

2025年5月以前に作成されたDBインスタンスについては、Webコンソールの`VIP追加`メニューをクリックしてVIPを追加できます。VIPを追加すると、既存の内部ドメインと内部(VIP)ドメインの両方が提供されます。ただし、フェイルオーバーが発生した場合、VIPは予備マスターを指すようになりますが、内部ドメインは場合によっては予備マスターを指さない可能性があります。そのため、VIPを追加した場合は、アプリケーションの接続情報を必ずVIPまたは内部(VIP)ドメインを使用するように修正してください。

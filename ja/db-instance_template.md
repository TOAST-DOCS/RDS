...
| 次回適用 | 次のメンテナンス期間に実行されるメンテナンス作業があります。 |
| 適用中 | メンテナンス作業が進行中です。 |
...
準備中のメンテナンス作業は、**[保留]** または **[削除]** ボタンをクリックして、メンテナンス期間から除外できます。ユーザーメンテナンス作業は削除され、Providerメンテナンス作業および自動メンテナンス作業は保留されます。保留中のメンテナンス作業は、**[即座に適用]** または **[次のメンテナンス期間に適用]** を選択して手動で適用できます。

<a id="maintenance-auto-minor-version-upgrade"></a>
#### マイナーバージョン自動アップグレード

マイナーバージョン自動アップグレードを使用すると、ユーザーが直接リクエストしなくても DB エンジンのマイナーバージョンが自動的にアップグレードされます。自動アップグレード対象バージョンがある場合、自動メンテナンス作業が登録され、次のメンテナンス期間に実行されます。

**設定**

DB インスタンスの作成および復元時に、メンテナンス領域で設定できます。デフォルトは有効です。作成後は、DB インスタンス一覧で DB インスタンスグループを選択し、**[修正]** をクリックして変更できます。

この設定は DB インスタンスグループ全体に同様に適用されます。Primary、Standby、Read Replica がそれぞれ異なる設定を持つことはできません。Read Replica を新たに作成した場合、グループの設定がそのまま適用されます。

**適用条件**

次の条件をすべて満たす DB インスタンスに適用されます。

| 条件                                                         | 満たさない場合         |
|--------------------------------------------------------------|----------------|
| ファミリーパラメータグループを使用                                          | 適用対象から除外されます。 |
| 現在の DB エンジンバージョンが自動アップグレード対象                               | 適用対象から除外されます。 |
| 予約または保留中の DB エンジンバージョンアップグレード作業なし                         | 次の周期に延期されます。  |
| Primary はすべての Read Replica が先にアップグレードされた状態                  | 次の周期に延期されます。  |

自動アップグレード対象バージョンは、同じメジャーバージョン内のマイナーバージョンに限定されます。メジャーバージョンアップグレードは自動的には実行されません。

ユーザーが予約した、または Provider が登録した DB エンジンバージョンアップグレード作業が残っている場合、自動メンテナンス作業を登録せずに譲ります。残っていた作業が処理されると、次の周期に再度登録されます。

!!! tip "ヒント"
    単一パラメータグループを使用する DB インスタンスには適用されません。自動アップグレードを使用するには、[ファミリーパラメータグループ](parameter-group/#family-parameter-group)に変更する必要があります。

**適用順序**

レプリケーション構成を維持するため、Read Replica を先にアップグレードしてから Primary をアップグレードします。高可用性 DB インスタンスの Primary と Standby は、1 つの作業として同時にアップグレードされます。この順序により、DB インスタンスグループ全体に反映されるまでに複数のメンテナンス期間がかかる場合があります。

**対象判定周期**

自動アップグレード対象かどうかは 1 日に 1 回判定されます。対象の場合は、次のメンテナンス期間に実行される自動メンテナンス作業が登録されます。

**作業除外**

登録された自動メンテナンス作業を今回のメンテナンス期間に実行したくない場合は、メンテナンスタブで **[保留]** を選択します。保留した作業は再度登録されません。**[次のメンテナンス期間に適用]** を選択すると、その時点で実行されます。

自動アップグレード自体を中断するには、DB インスタンスグループを修正してマイナーバージョン自動アップグレードを無効に変更します。無効に変更すると、まだ実行されていない自動メンテナンス作業も合わせて削除されます。
...
保留中のメンテナンスは、NHN Cloud が提供する Provider メンテナンス作業と、システムが自動的に登録した自動メンテナンス作業の一覧です。パラメータグループの変更の適用、ハイパーバイザー点検のためのマイグレーション、マイナーバージョンの自動アップグレードなどの作業が含まれます。

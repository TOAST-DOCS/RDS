...
## DB インスタンスグループ詳細 { #db-instance-group-details }

DB インスタンス一覧を**グループ**画面で表示した後、DB インスタンスグループを選択すると、グループの詳細情報を確認できます。グループ詳細画面には次のタブが表示されます。

| タブ             | 説明                                                                                                |
|---------------|---------------------------------------------------------------------------------------------------|
| 基本情報         | DB インスタンスグループ名と ID、高可用性構成、Primary および Standby の名前、Ping 設定を確認します。                                  |
| DB スキーマ & ユーザー | グループに属する DB インスタンスの DB スキーマとユーザーを管理します。DB スキーマとユーザー機能は、個別の DB インスタンス詳細ではなく、DB インスタンスグループ詳細で提供します。 |

<a id="db-schema-and-users"></a>
### DBスキーマ&ユーザー { #db-schema-and-users }

DBインスタンスグループの詳細の **[DB スキーマ & ユーザー]** タブでは、グループに属するデータベースのスキーマとユーザーを照会および制御できます。
...
## DB インスタンスグループの修正 { #modify-db-instance-group }
...
| 項目                 | 説明                                                                     |
|--------------------|------------------------------------------------------------------------|
| DB インスタンスグループ名      | 1〜100 文字の英字、数字、`-`、`_`、`.` を使用でき、最初の文字は英字である必要があります。             |
| Primary 名         | グループ名とは別に管理されます。                                                     |
| 高可用性の有無            | 単一構成は高可用性構成に切り替えることができ、高可用性構成は単一構成に切り替えることができます。                |
| Standby 名         | 高可用性を新たに設定するときに入力します。Primary 名と同じにすることはできず、名前のルールは Primary と同じです。      |
| Ping 間隔            | 高可用性構成で 1〜600 秒の範囲で設定します。                                            |
| Ping 方式            | 高可用性構成で `INSERT` または `SELECT` から選択します。                                 |
| DB スキーマ & ユーザー直接制御 | グループに属する DB インスタンスのスキーマとユーザー直接制御の使用有無を変更します。                           |

!!! danger "注意"
    * 高可用性を無効にすると、Standby が削除され、単一構成に切り替わります。Standby にのみ存在するデータや設定がないことを確認してから進めてください。
    * フェイルオーバーが進行中の DB インスタンスグループでは、高可用性の有無を変更することはできません。
    * プライベートネットワークでは、Primary 名を変更することはできません。また、既存の高可用性構成の Standby 名は変更できず、高可用性を新たに設定するときのみ Standby 名を入力できます。
    * 高可用性構成では、Primary と Standby の名前は異なる必要があります。

...
| dbVersion | Enum | Y | DBエンジンバージョン |
| dbEngineVersionFamily | String | N | DBエンジンバージョンファミリーコード（FAMILYタイプ作成時に必須: 指定するとファミリーパラメータグループが作成され、同じファミリーのすべてのマイナーバージョン DB インスタンスに共有適用可能） |
...
| parameters.templateRange.label | String | 区間の DB エンジンバージョン範囲 |
| createdYmdt | DateTime | 作成日時(YYYY-MM-DDThh:mm:ss.SSSTZD) |
| updatedYmdt | DateTime | 修正日時(YYYY-MM-DDThh:mm:ss.SSSTZD) |

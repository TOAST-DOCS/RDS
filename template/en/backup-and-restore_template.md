...
![db-instance-backup-en]({{url.cdn}}/24.03.12/db-instance-backup-en.png)
![backup-list-en]({{url.cdn}}/24.11.12/backup-list-en.png)
...
기존 백업 방식은 백업 애플리케이션이 VM 내에서 실행되어 백업 중 성능 저하가 발생할 수 있었으나, **스토리지 스냅샷 백업**은 **고가용성이 정상 상태로 활성화 되어 있는 경우**에 한해 Cinder Storage Snapshot을 활용하여 백업을 수행합니다.
스냅샷 생성 후 별도의 공용 VM에서 데이터 유효성 검사 및 파일 변환을 수행하여, 백업 중에도 고객 VM의 성능에 영향을 주지 않습니다.

**주요 특징**:
* **성능 유지**: 백업 수행 중에도 DB 인스턴스의 성능이 100% 유지됩니다.
* **안정성 향상**: 별도의 검증 프로세스를 통해 백업 데이터의 신뢰성을 보장합니다.
* **고가용성 일시 정지**: 스냅샷 생성 시점에는 데이터 일관성을 위해 고가용성 기능이 잠시 일시 정지될 수 있습니다.

### 요금 체계

스냅샷 백업은 기존 백업과 달리 백업 수행을 위해 사용된 리소스 비용이 별도로 청구됩니다.

| 구분 | 기존 백업 방식 | 스냅샷 백업 방식 |
| --- | --- | --- |
| **청구 방식** | 고객 VM 사용 요금에 포함 (별도 청구 없음) | **백업 전용 리소스 비용 별도 청구** |
| **과금 대상** | OBS 업로드 비용 (별도) | **백업 전용 VM + 볼륨 + 스냅샷 + OBS** |

* **백업 전용 VM 요금**: 백업 데이터의 검증 및 변환을 위해 일시적으로 생성되는 공용 VM의 사용료입니다.
    * 공용 리소스를 사용하더라도 **각 고객이 실제 사용한 시간에 대해 과금**됩니다.
...
![db-instance-snapshot-restoration-en]({{url.cdn}}/24.03.12/db-instance-snapshot-restoration-en.png)
...
![snapshot-restoration-en]({{url.cdn}}/24.03.12/snapshot-restoration-en.png)
...
* When the binary log is automatically deleted by {{engine.pascalCase}} based on the Binary log retention period
...
![point-in-time-restoration-list-en]({{url.cdn}}/24.03.12/point-in-time-restoration-list-en.png)
...
![point-in-time-restoration-01-en]({{url.cdn}}/24.03.12/point-in-time-restoration-01-en.png)

❶ Select a restore method.

![point-in-time-restoration-02-en]({{url.cdn}}/24.03.12/point-in-time-restoration-02-en.png)

❷ Select a restore time. You can restore to the most recent point in time, or you can enter a specific point in time.

![point-in-time-restoration-03-en]({{url.cdn}}/24.03.12/point-in-time-restoration-03-en.png)
...
![point-in-time-restoration-04-en]({{url.cdn}}/24.03.12/point-in-time-restoration-04-en.png)
...
(4) Delete all files in the {{engine.pascalCase}} data storage path.
...
(7) Start {{engine.pascalCase}} service. 
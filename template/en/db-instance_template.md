## Database > RDS for {{engine.pascalCase}} > DB Instance
...
* [마이너 버전 자동 업그레이드](db-instance/#auto-minor-version-upgrade)
...
<a id="auto-minor-version-upgrade"></a>
#### 마이너 버전 자동 업그레이드
...
이 설정은 DB 인스턴스 그룹 전체에 동일하게 적용됩니다. Primary, Standby, Read replica 가 서로 다른 설정을 가질 수 없으며, Read replica 를 새로 생성하면 그룹의 설정을 그대로 따릅니다.
...
| Primary 는 모든 Read replica 가 먼저 업그레이드된 상태       | 다음 주기로 미뤄집니다.   |

자동 업그레이드 대상 버전은 같은 메이저 버전 안의 마이너 버전으로 한정됩니다. 메이저 버전 업그레이드는 자동으로 수행되지 않습니다.

사용자가 예약했거나 Provider 가 등록한 DB 엔진 버전 업그레이드 작업이 남아 있으면 자동 유지 관리 작업을 등록하지 않고 양보합니다. 남아 있던 작업이 처리되면 다음 주기에 다시 등록됩니다.
...
복제 구성을 유지하기 위해 Read replica 를 먼저 업그레이드한 뒤 Primary 를 업그레이드합니다. 고가용성 DB 인스턴스의 Primary 와 Standby 는 하나의 작업으로 함께 업그레이드됩니다. 이 순서 때문에 DB 인스턴스 그룹 전체에 반영되기까지 여러 유지 관리 기간이 걸릴 수 있습니다.
...
![rds_planed_migration_0]({{url.cdn}}/planned_migration_alarm/26.01.13/image0_en.png)
...
![rds_planed_migration_1]({{url.cdn}}/planned_migration_alarm/26.01.13/image1_en.png)
...
![rds_planed_migration_2]({{url.cdn}}/planned_migration_alarm/26.01.13/image2_en.png)
...
![rds_planed_migration_3]({{url.cdn}}/planned_migration_alarm/26.01.13/image3_en.png)

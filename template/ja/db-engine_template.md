...
DBインスタンスが高可用性で構成されている場合、Standbyのエンジンバージョンを先にアップグレードした後、フェイルオーバーを使用してStandbyをPrimaryに昇格させます。フェイルオーバーは、Primaryのサービスを一時的に中断させるため、ユーザーが望むタイミングでフェイルオーバーを開始できます。

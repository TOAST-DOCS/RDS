...
## Database > RDS for MariaDB > DB 인스턴스 { #database-rds-for-enginepascalcase-db-instance }
...
* DB 인스턴스 이름은 1~100자의 영문자, 숫자, 일부 기호(-, _, .)만 사용할 수 있으며, 첫 번째 글자는 영문자만 사용할 수 있습니다.
...
DB 엔진에 관한 자세한 내용은 [DB 엔진](db-engine-gov/)에서 확인할 수 있습니다.
...
고가용성 DB 인스턴스는 가용성과 데이터 내구성을 높이고, 장애를 허용하는 데이터베이스를 제공합니다. 고가용성 DB 인스턴스는 Primary, Standby로 구성되며 서로 다른 가용성 영역에 생성됩니다. Standby는 장애에 대비한 DB 인스턴스로 평소에는 사용할 수 없습니다. 고가용성 DB 인스턴스는 Standby에서 백업이 수행되기 때문에 백업으로 인한 성능 저하를 회피할 수 있습니다. 고가용성 DB 인스턴스가 제공하는 여러 기능은 [고가용성 DB 인스턴스](db-instance-gov/#ha-db-instance)에서 확인할 수 있습니다.
...
파라미터 그룹은 DB 인스턴스에 설치된 데이터베이스를 설정할 수 있는 파라미터의 집합입니다. DB 인스턴스 생성 시 반드시 하나의 파라미터 그룹을 선택해야 합니다. 파라미터 그룹은 생성 이후에도 자유롭게 변경할 수 있습니다. 자세한 파라미터 그룹 내용은 [파라미터 그룹](parameter-group-gov/) 항목을 참고합니다.

<a id="db-security-group"></a>
### DB 보안 그룹 { #db-security-group }

DB 보안 그룹은 외부 침입에 대비해 접속을 제한하는 데 사용합니다. 송수신 트래픽에서 특정 포트 범위 또는 데이터베이스 포트에 접근을 허용할 수 있습니다. DB 인스턴스에 여러 개의 DB 보안 그룹을 적용할 수 있습니다. 자세한 DB 보안 그룹 내용은 [DB 보안 그룹](db-security-group-gov/) 항목을 참고합니다.

<a id="backup"></a>
### 백업 { #backup }

DB 인스턴스의 데이터베이스를 주기적으로 백업하도록 설정하거나, 콘솔에서 원하는 시기에 백업을 생성할 수 있습니다. 백업이 수행되는 동안 성능 저하가 발생할 수 있습니다. 서비스에 영향을 주지 않으려면 서비스 부하가 적은 시간에 백업하기를 권장합니다. 백업으로 인한 성능 저하를 원치 않으면 고가용성 구성을 사용하거나, 이전 백업 이후 데이터의 증분만 백업할 수 있으며, Read Replica에서 백업을 수행할 수 있습니다. 백업 파일은 내부 백업 스토리지에 저장되며, 백업 용량에 따라 과금됩니다. 필요한 경우 NHN Cloud의 Object Storage로 내보낼 수 있습니다. 예상치 못한 장애에 대비해 주기적으로 백업을 수행하도록 설정하기를 권장합니다. 자세한 백업 내용은 [백업 및 복원](backup-and-restore-gov/) 항목을 참고합니다.
...
유지 관리 작업은 사용자 유지 관리 작업, Provider 유지 관리 작업, 자동 유지 관리 작업으로 구분됩니다.
...
설정에 따라 시스템이 자동으로 등록하는 작업입니다.

* [마이너 버전 자동 업그레이드](db-instance-gov/#maintenance-auto-minor-version-upgrade)
...
| 다음 적용 | 다음 유지 관리 기간에 실행될 유지 관리 작업이 있습니다.                                   |
| 적용 중  | 유지 관리 작업이 진행 중입니다.                                                 |
| 필수    | 필수 유지 관리 작업이 있습니다. 미루더라도 결국 적용됩니다.                                 |
| 사용 가능 | 자동으로 실행되지 않아 사용자가 적용하거나 예약해야 하는 유지 관리 작업만 있습니다. 보류한 작업도 여기에 해당합니다. |
...
준비 중인 유지 관리 작업은 **보류** 또는 **삭제** 버튼을 클릭하여 유지 관리 기간에서 제외할 수 있습니다. 사용자 유지 관리 작업은 삭제되고, Provider 유지 관리 작업과 자동 유지 관리 작업은 보류됩니다. 보류 중인 유지 관리 작업은 **즉시 적용** 또는 **다음 유지 관리 기간에 적용**을 선택하여 수동으로 적용할 수 있습니다.

<a id="maintenance-auto-minor-version-upgrade"></a>
#### 마이너 버전 자동 업그레이드

마이너 버전 자동 업그레이드를 사용하면 사용자가 직접 요청하지 않아도 DB 엔진의 마이너 버전이 자동으로 업그레이드됩니다. 자동 업그레이드 대상 버전이 있으면 자동 유지 관리 작업이 등록되고, 다음 유지 관리 기간에 실행됩니다.

**설정**

DB 인스턴스 생성 및 복원 시 유지 관리 영역에서 설정할 수 있으며, 기본값은 사용입니다. 생성한 뒤에는 DB 인스턴스 목록에서 DB 인스턴스 그룹을 선택한 뒤 **수정**을 클릭하여 변경할 수 있습니다.

이 설정은 DB 인스턴스 그룹 전체에 동일하게 적용됩니다. Primary, Standby, Read Replica가 서로 다른 설정을 가질 수 없으며, Read Replica를 새로 생성하면 그룹의 설정을 그대로 따릅니다.

**적용 조건**

다음 조건을 모두 만족하는 DB 인스턴스에 적용됩니다.

| 조건                                     | 만족하지 않는 경우     |
|----------------------------------------|----------------|
| 패밀리 파라미터 그룹 사용                         | 적용 대상에서 제외됩니다. |
| 현재 DB 엔진 버전이 자동 업그레이드 대상               | 적용 대상에서 제외됩니다. |
| 예약 또는 보류 중인 DB 엔진 버전 업그레이드 작업 없음       | 다음 주기로 미뤄집니다.  |
| Primary는 모든 Read Replica가 먼저 업그레이드된 상태 | 다음 주기로 미뤄집니다.  |

자동 업그레이드 대상 버전은 같은 메이저 버전 안의 마이너 버전으로 한정됩니다. 메이저 버전 업그레이드는 자동으로 수행되지 않습니다.

사용자가 예약했거나 Provider가 등록한 DB 엔진 버전 업그레이드 작업이 남아 있으면 자동 유지 관리 작업을 등록하지 않고 양보합니다. 남아 있던 작업이 처리되면 다음 주기에 다시 등록됩니다.

!!! tip "알아두기"
    단일 파라미터 그룹을 사용하는 DB 인스턴스에는 적용되지 않습니다. 자동 업그레이드를 사용하려면 [패밀리 파라미터 그룹](parameter-group-gov/#family-parameter-group)으로 변경해야 합니다.

**적용 순서**

복제 구성을 유지하기 위해 Read Replica를 먼저 업그레이드한 뒤 Primary를 업그레이드합니다. 고가용성 DB 인스턴스의 Primary와 Standby는 하나의 작업으로 함께 업그레이드됩니다. 이 순서 때문에 DB 인스턴스 그룹 전체에 반영되기까지 여러 유지 관리 기간이 걸릴 수 있습니다.

**대상 판정 주기**

자동 업그레이드 대상 여부는 매일 한 번 판정하며, 대상이면 다음 유지 관리 기간에 실행될 자동 유지 관리 작업을 등록합니다.

**작업 제외**

등록된 자동 유지 관리 작업을 이번 유지 관리 기간에 실행하고 싶지 않으면 유지 관리 탭에서 **보류**를 선택합니다. 보류한 작업은 다시 등록되지 않으며, **다음 유지 관리 기간에 적용**을 선택하면 그때 실행됩니다.

자동 업그레이드 자체를 중단하려면 DB 인스턴스 그룹을 수정하여 마이너 버전 자동 업그레이드를 사용 안 함으로 변경합니다. 사용 안 함으로 변경하면 아직 실행되지 않은 자동 유지 관리 작업도 함께 삭제됩니다.
...
DB 인스턴스 생성 시 기본 알림을 설정할 수 있습니다. 기본 알림을 설정하면 `{DB 인스턴스 이름}-default` 이름으로 새로운 알림 그룹이 생성되며 아래 알림 항목이 자동으로 설정됩니다. 기본 알림으로 생성된 알림 그룹은 자유롭게 수정, 삭제할 수 있습니다. 자세한 알림 그룹 내용은 [알림 그룹](notification-gov/) 항목을 참고합니다.
...
![db-instance-list_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-list_ko.png)
...
![db-instance-filter_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-filter_ko.png)
...
![db-instance-detail-schema_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-schema_ko.png)
...
![db-instance-detail-schema-delete-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-schema-delete-ko.png)
...
![db-instance-detail-user-create-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-user-create-ko.png)
...
![db-instance-detail-user-modify-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-user-modify-ko.png)
...
![db-instance-detail-user-delete-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-user-delete-ko.png)
...
![db-instance-detail_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail_ko.png)
...
![db-instance-detail-log_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-log_ko.png)
...
❹ 바이너리 로그(binary log)는 2가지 형태로 내려받을 수 있습니다. **가져오기**를 클릭하면 바이너리 로그 형태를 선택할 수 있는 팝업 화면이 나타납니다.
...
![db-instance-detail-maintenance_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-maintenance_ko.png)
...
Provider 유지 관리 작업과 자동 유지 관리 작업은 보류 중인 유지 관리 목록으로 이동합니다. 보류 중인 유지 관리 목록에서 다시 준비 중인 유지 관리 작업으로 이동할 수 있습니다.

<a id="db-instance-details-maintenance-pending-maintenance"></a>
#### 보류 중인 유지 관리

보류 중인 유지 관리는 NHN Cloud에서 제공하는 Provider 유지 관리 작업과 시스템이 자동으로 등록한 자동 유지 관리 작업 목록입니다. 파라미터 그룹 변경 사항 적용, 하이퍼바이저 점검을 위한 마이그레이션, 마이너 버전 자동 업그레이드 등의 작업이 포함됩니다.
...
![db-instance-detail-maintenance-immediately_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-maintenance-immediately_ko.png)

**다음 유지 관리 기간에 적용**: 선택한 유지 관리 작업을 다음 유지 관리 기간에 수행합니다. **확인**을 클릭하면 준비 중인 유지 관리 목록으로 이동합니다.
![db-instance-detail-maintenance-schedule_ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-detail-maintenance-schedule_ko.png)
...
![modify-ha-popup-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/modify-ha-popup-ko.png)

❶ 유지 관리 기능으로 **다음 유지 관리 기간에 적용** 또는 **즉시 적용**을 선택해 DB 인스턴스 수정을 수행할 수 있습니다.
❷ 장애 조치를 이용한 재시작을 사용하지 않으면 Primary와 Standby에 변경 사항을 순차적으로 적용한 후 DB 인스턴스를 재시작합니다. 자세한 사항은 고가용성 DB 인스턴스의 [수동 장애 조치 항목](db-instance-gov/#manual-failover)을 참고합니다.
...
![db-instance-os-upgrade-ko.png](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-os-upgrade-ko.png)
...
![db-instance-os-upgrade-single-popup-ko.png](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-os-upgrade-simple-popup-ko.png)

고가용성 DB 인스턴스의 운영체제 버전 업그레이드 버튼을 클릭하면 아래와 같은 팝업 화면이 나타납니다. 자세한 사항은 고가용성 DB 인스턴스의 [수동 장애 조치 항목](db-instance-gov/#manual-failover)을 참고합니다.
![os-upgrade-ha-popup-ko.png](https://static.toastoven.net/prod_rds/mariadb/26.01.13/os-upgrade-ha-popup-ko.png)
...
장애 상황에 대비하여 DB 인스턴스의 데이터베이스를 복구할 수 있도록 미리 준비할 수 있습니다. 필요할 때마다 콘솔에서 백업을 수행하거나, 주기적으로 백업이 수행되도록 설정할 수 있습니다. 자세한 사항은 [백업](backup-and-restore-gov/#overview) 항목을 참고합니다.

<a id="restoration"></a>
## 복원 { #restoration }

백업을 이용하여 원하는 시점으로 데이터를 복원할 수 있습니다. 복원 시에는 항상 새로운 DB 인스턴스가 생성되며, 기존 DB 인스턴스에 복원할 수 없습니다. 자세한 사항은 [복원](backup-and-restore-gov/#restore) 항목을 참고합니다.
...
스토리지 자동 확장을 사용하려면 DB 인스턴스 생성 및 수정 시 **스토리지 자동 확장**을 활성화해야 합니다.

스토리지 자동 확장을 활성화하면 세 가지 옵션을 설정할 수 있습니다.
* 스토리지 자동 확장 조건: 스토리지 사용률이 설정한 값 이상으로 5분 이상 지속될 때 자동으로 스토리지를 확장합니다.
* 스토리지 자동 확장 최댓값: 스토리지 자동 확장으로 확장될 수 있는 최대 크기입니다.
* 스토리지 자동 확장 쿨다운: 스토리지 자동 확장 기능이 한 번 실행된 후, 다시 기능이 활성화되기까지의 시간을 설정합니다.

스토리지 자동 확장 기능이 실행될 때의 증가량은 다음 값 중 가장 큰 값으로 설정됩니다.
* 10GB
* 스토리지 크기의 10%
...
![db-instance-list-parameter-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-list-parameter-ko.png)
...
![db-instance-parameter-ha-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-parameter-ha-ko.png)
...
외부 MariaDB 백업 파일을 NHN Cloud의 Object Storage에 업로드하여 RDS for MariaDB의 DB 인스턴스로 복원할 수 있습니다. 자세한 사항은 [외부 MariaDB 백업을 이용한 복원](backup-and-restore-gov/#restore-from-external) 항목을 참고합니다.
...
백업 후 백업 파일을 Object Storage로 내보낼 수 있습니다. 자세한 사항은 [백업 내보내기](backup-and-restore-gov/#export) 항목을 참고합니다.
...
### Read Replica 생성 { #create-read-replications }
...
!!! danger "주의"
    Primary의 데이터베이스 크기에 비례하여 Read Replica 생성 시간이 늘어날 수 있습니다.
    백업이 수행되는 DB 인스턴스는 Read Replica 생성 과정에서 스토리지 I/O 성능 하락이 있을 수 있습니다.
...
![db-instance-replica-create-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-replica-create-ko.png)

❶ 원본 DB 인스턴스를 선택한 뒤 **Read Replica 생성**을 클릭하면
...
Read Replica의 파라미터 그룹을 선택할 때 복제 관련 설정 변경이 필요 없다면 원본 DB 인스턴스와 동일한 파라미터 그룹을 선택하기를 권장합니다. 자세한 파라미터 그룹 내용은 [파라미터 그룹](parameter-group-gov/) 항목을 참고합니다.

<a id="create-read-replications-db-security-group"></a>
#### DB 보안 그룹

Read Replica에 적용할 DB 보안 그룹을 선택합니다. 복제에 필요한 규칙은 자동으로 적용되기 때문에 DB 보안 그룹에 별도로 복제 관련 규칙을 추가할 필요가 없습니다. 자세한 DB 보안 그룹 내용은 [DB 보안 그룹](db-security-group-gov/) 항목을 참고합니다.

<a id="create-read-replications-backup"></a>
#### 백업

Read Replica의 백업 설정을 선택합니다. 자세한 백업 내용은 [백업 및 복원](backup-and-restore-gov/) 항목을 참고합니다.
...
### Read Replica 강제 승격 { #force-promotion-of-read-replicas }
...
Read Replica의 복제 문제를 해결할 수 없는 경우 재구축으로 정상 상태로 복원할 수 있습니다. 이 과정에서 Read Replica의 모든 데이터베이스를 삭제하고, Primary 데이터베이스를 기반으로 새롭게 재구축합니다. 재구축하는 동안 Read Replica는 사용할 수 없습니다. Read Replica를 재구축하려면 복제 그룹에 속한 DB 인스턴스 중 테이블 잠금 사용 옵션으로 생성된 백업 파일 및 바이너리 로그(binary log)가 필요합니다. 백업 파일이 없는 경우 동작 및 주의 사항은 [Read Replica 생성](#create-read-replications) 항목을 참고합니다.
...
MariaDB을 재시작하거나 고가용성 DB 인스턴스를 수동으로 장애 조치하려면 DB 인스턴스를 재시작할 수 있습니다. 재시작 시간을 최소화하기 위해 서비스 부하가 낮은 시간대에 수행하기를 권장합니다. 고가용성 DB 인스턴스는 장애 조치를 이용한 재시작을 사용하지 않으면 Standby를 먼저 재시작한 뒤 Primary를 재시작합니다. 장애 조치 기능을 이용한 재시작은 [수동 장애 조치](#manual-failover) 항목을 참고합니다.

DB 인스턴스 재시작을 하려면 콘솔에서

![db-instance-restart-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-restart-ko.png)
...
![db-instance-restart-force-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-restart-force-ko.png)
...
![db-instance-deletion-protection-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-deletion-protection-ko.png)
...
장애 조치 진행 단계는 다음과 같습니다. 되돌릴 수 있는 단계에서만 **장애 조치 되돌리기** 버튼이 나타납니다.

| 진행 단계                 | 설명                                                              | 되돌리기 |
| --------------------- | --------------------------------------------------------------- | ---- |
| Failed Over Primary 차단 | 장애가 발생한 Primary의 접속을 차단하고 데이터베이스 엔진을 종료합니다.                       | 가능   |
| 복제 로그 반영              | Standby가 아직 반영하지 못한 복제 로그를 반영합니다.                                | 가능   |
| Endpoint 전환           | 내부 도메인과 VIP가 Standby를 가리키도록 변경하고 쓰기를 허용합니다.                      | 불가   |
| 복제 재구성                | 남은 복제본이 새 Primary를 바라보도록 복제를 다시 설정합니다.                           | 불가   |
| 메타 정보 정리              | 장애 조치 결과를 DB 인스턴스 정보에 반영합니다.                                     | 불가   |
| 되돌리는 중                | 요청한 장애 조치 되돌리기를 처리합니다.                                           | 불가   |

복제 로그 반영 단계에서는 반영이 얼마나 남았는지를 복제 로그 반영 진행률 열에서 확인할 수 있습니다. 표시되는 형태는 다음과 같습니다.

| 표시              | 의미                                                    |
| --------------- | ----------------------------------------------------- |
| `62% (약 40초 남음)` | 62%까지 반영했으며 남은 반영에 약 40초가 걸립니다.                       |
| `7%`            | 7%까지 반영했으나 남은 시간을 추정할 표본이 아직 모이지 않았습니다.               |
| `7% (진행 없음)`    | 7%까지 반영한 뒤 1분이 넘도록 반영이 진행되지 않았습니다.                    |
| `마무리 중`         | 복제 로그를 모두 반영했고 다음 단계로 넘어가기 위한 정리를 수행 중입니다.            |
...
장애 조치를 되돌리면 진행 중이던 장애 조치를 중단하고 장애가 발생한 Primary를 재시작하여 다시 Primary로 사용합니다. 차단했던 사용자 보안 그룹의 연결을 복구하고 복제본의 복제를 다시 시작합니다. 되돌리기는 Endpoint 전환 전까지만 할 수 있으므로 Standby가 새 Primary로 전환되어 쓰기를 받은 적이 없으며, 되돌린 뒤에도 기존 Primary의 데이터를 그대로 사용합니다.

!!! danger "주의"
    장애의 원인에 따라 장애가 발생한 Primary를 정상적으로 기동하지 못할 수 있습니다. 이 경우 되돌리기는 실패하며 DB 인스턴스는 장애 조치 중 상태로 남습니다.
    장애가 발생한 Primary에 접속할 수 없으면 고객센터로 문의해야 합니다.

장애 조치를 되돌린 뒤에도 고가용성 기능은 중단된 상태로 남습니다. 따라서 되돌리기가 완료되면 **고가용성 다시 시작**으로 고가용성 기능을 재개하거나, Standby의 복제가 크게 지연되었다면 [Standby 재구축](#rebuild-candidate-master)을 수행해야 합니다. 고가용성 기능이 이미 중단된 상태이므로 고가용성 일시 중지는 수행할 수 없습니다.

다음과 같은 경우에는 되돌리기 요청이 거부되며 화면에 사유가 표시됩니다.

| 사유                                             | 조치                                                     |
| ---------------------------------------------- | ------------------------------------------------------ |
| 장애 조치가 이미 되돌릴 수 없는 단계로 진행되었습니다.                 | 장애 조치가 완료될 때까지 기다립니다.                                   |
| 이미 장애 조치 되돌리기를 처리하고 있습니다.                       | 되돌리기가 완료될 때까지 기다립니다.                                    |
| 진행 중인 장애 조치가 없습니다.                              | 화면을 새로 고쳐 최신 상태를 확인합니다.                                 |
| 장애가 발생한 Primary DB 인스턴스가 응답하지 않아 되돌릴 수 없습니다.     | 장애 조치가 완료될 때까지 기다린 후 Failed Over Primary를 복구하거나 재구축합니다. |
...
### Failed Over Primary 복구 { #recover-failed-over-master }
...
![db-instance-failover-repair-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-failover-repair-ko.png)

❶ 복구를 원하는 Failed Over Primary를 선택 후 드롭다운 메뉴에서 **Failed Over Primary 복구** 메뉴를 클릭합니다.

<a id="rebuild-failed-over-master"></a>
### Failed Over Primary 재구축 { #rebuild-failed-over-master }
...
!!! danger "주의"
    Primary의 데이터베이스 크기에 비례하여 Failed Over Primary 재구축 시간이 늘어날 수 있습니다.
    백업이 수행되는 DB 인스턴스는 Failed Over Primary 재구축 과정에서 스토리지 I/O 성능 하락이 있을 수 있습니다.
...
![db-instance-failover-rebuild-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-failover-rebuild-ko.png)

❶ 재구축을 원하는 Failed Over Primary를 선택 후 드롭다운 메뉴에서 **Failed Over Primary 재구축** 메뉴를 클릭합니다.

<a id="separate-failed-over-master"></a>
### Failed Over Primary 분리 { #separate-failed-over-master }
...
![db-instance-failover-split-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-failover-split-ko.png)
...
![db-instance-ha-wait-manual-failover-ko](https://static.toastoven.net/prod_rds/mariadb/26.01.13/db-instance-ha-wait-manual-failover-ko.png)
...
## MariaDB Procedure { #enginepascalcase-procedure }
...
* 플로팅 IP를 생성하여 NHN Cloud RDS 인스턴스에 연결합니다.
* 다음 mysqldump 명령어로 외부에서 데이터를 가져옵니다.
...
* 데이터를 내보낼 NHN Cloud RDS Primary 또는 Read Replica 인스턴스를 준비합니다.
* 플로팅 IP를 생성하여 데이터를 내보낼 NHN Cloud RDS 인스턴스에 연결합니다.
* 아래 명령어로 NHN Cloud RDS 인스턴스에서 데이터를 파일로 내보냅니다.
* Primary RDS 인스턴스에서 내보낼 경우
...
* Read Replica RDS 인스턴스에서 내보낼 경우
...
* 백업된 파일을 열어 주석에 적힌 MASTER_LOG_FILE 및 MASTER_LOG_POS를 따로 기록합니다.
* NHN Cloud RDS 인스턴스에서 데이터를 백업할 외부 로컬 클라이언트 또는 DB가 설치된 컴퓨터의 용량이 충분한지 확인합니다.
* 외부 DB의 my.cnf(Windows의 경우 my.ini) 파일에 다음과 같은 옵션을 추가합니다.
* server-id는 NHN Cloud RDS 인스턴스의 파라미터 항목의 server-id와 다른 값으로 입력합니다.
...
* 새롭게 복제를 설정하기에 앞서 혹시 존재할 수 있는 기존 복제 정보를 초기화하기 위해 다음 쿼리를 실행합니다. 이때, RESET SLAVE를 실행할 경우 기존 복제 정보가 초기화됩니다.
...
* 복제에 사용할 계정 정보와 앞에서 기록해 둔 MASTER_LOG_FILE과 MASTER_LOG_POS를 사용하여 외부 DB에 아래와 같이 쿼리를 실행합니다.
...
* 아래 명령어로 외부 MariaDB 인스턴스에서 데이터를 백업합니다.
* 외부 MariaDB 인스턴스(마스터)에서 가져올 경우
...
* 외부 MariaDB 인스턴스(슬레이브)에서 가져올 경우
...
* NHN Cloud RDS 인스턴스에서 데이터를 백업할 클라이언트나 컴퓨터의 용량이 충분한지 확인합니다.
* 외부 DB의 my.cnf(Windows의 경우 my.ini) 파일에 다음 옵션을 추가합니다.
* server-id는 NHN Cloud RDS 인스턴스의 파라미터 항목의 server-id와 다른 값으로 입력합니다.
...
* 외부 DB를 재시작합니다.
* 외부 네트워크를 통해 가져오면(import) 오래 걸릴 수 있어 내부 NHN Cloud Image를 생성하고 백업 파일을 복사한 후, NHN Cloud로 가져오기를 권장합니다.
* 백업된 파일을 다음 명령어로 NHN Cloud RDS에 입력합니다.
...
* 복제에 사용할 계정 정보와 앞에서 기록해 둔 MASTER_LOG_FILE, MASTER_LOG_POS를 사용하여 NHN Cloud RDS에 다음과 같이 쿼리를 실행합니다.
...
* 외부 DB와 NHN Cloud RDS 인스턴스의 원본 데이터가 같아지면 다음 명령으로 복제를 종료합니다.
...
DB 구성에 따라 특정 DB 인스턴스를 선택하여 마이그레이션 시, 연관된 DB 인스턴스(예: Read Replica 인스턴스)도 점검 대상이면 같이 마이그레이션을 수행합니다.
...
* 리모트 노드로의 송신을 허용하는 설정이 필요합니다.
    ...
    * 자세한 사항은 [DB 보안 그룹](db-security-group-gov/) 항목을 참고합니다.
* 로컬 노드 역할의 RDS에 Read Replica를 추가한 구성으로 사용할 경우 파라미터의 replicate-ignore-table에 federated 설정된 테이블을 명시해야 합니다.
    * Read Replica를 구성할 경우 federated 테이블 또한 복제되어 Primary와 Read Replica가 리모트 노드를 함께 바라봅니다.
    * 이 경우 Primary에 수행한 데이터 입력이 federated 설정에 따라 리모트 노드에도 수행되고, Read Replica에서도 마찬가지로 동일한 입력이 수행되어 중복 키 오류 등으로 인한 복제 중단이 발생할 수 있습니다.
    * Read Replica가 federated 테이블은 복제하지 않도록 replicate-ignore-table에 설정이 필요합니다.
...
![patch-security-list-ko](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-list-ko.png)

❶ 보안 패치 유지 관리 **보기** 버튼 클릭
❷ 현재 DB 이미지에 해당하는 보안 취약점 정보를 확인할 수 있습니다.

![patch-security-detail-ko](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-detail-ko.png)

보안 패치를 적용하면 해결할 수 있는 보안 취약점 정보를 확인할 수 있습니다.

![patch-security-popup-ko](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-popup-ko.png)
...
![patch-security-maintenance-ko](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-maintenance-ko.png)
...
![patch-security-running-ko](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-running-ko.png)

DB 인스턴스에 보안 패치가 적용되는 동안에는 아무런 조작을 할 수 없습니다.
보안 패치가 정상적으로 완료되지 않으면 자동으로 재시도되며, 반복적으로 실패할 경우 관리자에게 보고되어 NHN Cloud에서 별도로 연락드립니다.

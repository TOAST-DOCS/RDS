...
## Database > RDS for MariaDB > DBインスタンス { #database-rds-for-enginepascalcase-db-instance }
...
外部MariaDBのバックアップファイルをNHN CloudのObject Storageにアップロードして、RDS for MariaDBのDBインスタンスに復元することができます。詳細は、[外部MariaDBバックアップを利用した復元](backup-and-restore-gov/#restore-from-external)を参照してください。
...
MariaDBを再起動したり、高可用性DBインスタンスを手動でフェイルオーバーしたい場合、DBインスタンスを再起動できます。再起動時間を最小化するため、サービス負荷が低い時間帯に行うことをお勧めします。高可用性DBインスタンスの場合、フェイルオーバーを利用した再起動を使用しない場合、Standbyを先に再起動した後、Primaryを再起動します。フェイルオーバー機能を利用した再起動の場合、[手動フェイルオーバー](#manual-failover)項目を参照してください。
...
!!! danger "注意"
    複製用アカウントが複製対象(Master) MariaDBに作成されている必要があります。

<a id="tcrdsreplchangesource-after-84"></a>
### tcrds_repl_changesource (8.4以降) { #tcrdsreplchangesource-after-84 }

* レプリケーションを利用して外部のMariaDB DBをNHN Cloud RDSにインポートする際に使用します。
...
* パラメータの説明
      ...
      * master_instance_port:レプリケーション元(マスター)サーバーのMariaDBポート
      * user_id_for_replication:レプリケーション元(マスター)サーバーのMariaDBに接続するためのレプリケーション用アカウント
...
!!! danger "注意"
    レプリケーション用アカウントがレプリケーション元(マスター) MariaDBに作成されている必要があります。
...
例) MariaDB error code 1236 (ER_SOURCE_FATAL_ERROR_READING_BINLOG): Got fatal error from source when reading data from binary log
...
![patch-security-list-ja](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-list-ja.png)

❶ セキュリティパッチメンテナンスの**表示**ボタンをクリック
❷ 現在のDBイメージに該当する、セキュリティの脆弱性情報を確認できます。

![patch-security-detail-ja](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-detail-ja.png)

セキュリティパッチを適用した際に解決されるセキュリティの脆弱性情報を確認できます。

![patch-security-popup-ja](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-popup-ja.png)
...
![patch-security-maintenance-ja](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-maintenance-ja.png)
...
![patch-security-running-ja](https://static.toastoven.net/prod_rds/mariadb/26.05.12/patch-security-running-ja.png)

...
## Database > RDS for MariaDB > 分析 { #database-rds-for-enginepascalcase-analysis }
...
!!! tip "参考"
    収集されたデータは、内部バックアップストレージに保存され、別途課金されることはありません。
...
!!! tip "参考"
    DBインスタンスの修正画面でSlow Query分析機能を使用しないように設定できます。
...
!!! danger "注意"
    slow_query_logパラメータの値が0の場合、Slow Queryログを残さず、分析結果も収集しません。

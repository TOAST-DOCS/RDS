...
패밀리 파라미터 그룹을 사용하면 마이너 버전이 서로 다른 DB 인스턴스에 같은 파라미터 그룹을 함께 적용할 수 있어, 마이너 버전마다 파라미터 그룹을 따로 만들고 관리할 필요가 없습니다. 자세한 내용은 [패밀리 파라미터 그룹](parameter-group-gov/#family-parameter-group)을 참고합니다.
...
!!! tip "알아두기"
    [마이너 버전 자동 업그레이드](db-instance-gov/#maintenance-auto-minor-version-upgrade)는 패밀리 파라미터 그룹을 사용하는 DB 인스턴스에만 적용됩니다.
...
!!! danger "주의"
    * 각 단계에서 파라미터 그룹 변경 후에는 반드시 [파라미터 그룹 변경 사항 적용](parameter-group-gov/#apply)을 수행해야 합니다.

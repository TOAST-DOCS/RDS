...
size of backup storage. You can export to NHN Cloud Object Storage if necessary. To prepare for unexpected failures, we recommend that you set up backups to be conducted periodically. For more details on backup, see [Backup and Restore](backup-and-restore-ngoic/).
...
❷ If you do not use 'Reboot with Failover', changes will be applied sequentially to the Primary and Standby, followed by a restart of the DB instance. For more details, please refer to the [Manual Failover section](db-instance-ngoic/#manual-failover) for High Availability DB instances.
...
If restart with failover is not enabled, the DB instance is restarted after the changes are sequentially applied to the Primary and Standby. For more information, refer to [Manual failover item](db-instance-ngoic/#manual-failover) in a high availability DB instance.

<a id="recover-from-backup-in-object-storage"></a>
## Recover from backup in Object Storage { #recover-from-backup-in-object-storage }

You can upload an external MySQL backup file to NHN Cloud Object Storage to restore it to DB instance in RDS for MySQL. For more information, refer to [Restore with External MySQL Backup](backup-and-restore-ngoic/#restore-from-external).
...
After backup, you can export backup files to Object Storage. Refer to [Export Backup](backup-and-restore-ngoic/#export) for more information.

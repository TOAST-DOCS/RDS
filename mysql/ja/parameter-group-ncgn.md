...
ファミリーパラメータグループを使用すると、マイナーバージョンが異なるDBインスタンスに同じパラメータグループをまとめて適用できます。マイナーバージョンごとにパラメータグループを個別に作成・管理する必要がありません。詳細については、[ファミリーパラメータグループ](parameter-group-ncgn/#family-parameter-group)を参照してください。
...
!!! tip "参考"
    [マイナーバージョン自動アップグレード](db-instance-ncgn/#maintenance-auto-minor-version-upgrade)は、ファミリーパラメータグループを使用するDBインスタンスにのみ適用されます。
...
!!! danger "注意"
    * 各手順でパラメータグループを変更した後は、必ず[パラメータグループの変更内容を適用](parameter-group-ncgn/#apply)を実行する必要があります。

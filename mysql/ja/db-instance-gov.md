...
メンテナンス作業は、ユーザーメンテナンス作業、Providerメンテナンス作業、自動メンテナンス作業に分類されます。
...
設定に応じて、システムが自動的に登録する作業です。

* [マイナーバージョン自動アップグレード](db-instance-gov/#maintenance-auto-minor-version-upgrade)
...
| 状態 | 説明 |
|-------|-------------------------------------------------------------------|
| なし | 予約済みおよび保留中のメンテナンス作業はありません。 |
| 次回適用 | 次回のメンテナンス期間に実行されるメンテナンス作業があります。 |
| 適用中 | メンテナンス作業が進行中です。 |
| 必須 | 必須のメンテナンス作業があります。延期しても、最終的には適用されます。 |
| 利用可能 | 自動的に実行されないため、ユーザーが適用または予約する必要があるメンテナンス作業のみがあります。保留した作業もここに含まれます。 |
...
準備中のメンテナンス作業は、**保留**または**削除**ボタンをクリックして、メンテナンス期間から除外できます。ユーザーメンテナンス作業は削除され、Providerメンテナンス作業および自動メンテナンス作業は保留されます。保留中のメンテナンス作業は、**即時適用**または**次回のメンテナンス期間に適用**を選択して手動で適用できます。

<a id="maintenance-auto-minor-version-upgrade"></a>
#### マイナーバージョン自動アップグレード

マイナーバージョン自動アップグレードを使用すると、ユーザーが直接リクエストしなくてもDBエンジンのマイナーバージョンが自動的にアップグレードされます。自動アップグレード対象バージョンがある場合、自動メンテナンス作業が登録され、次回のメンテナンス期間に実行されます。

**設定**

DBインスタンスの作成および復元時に、メンテナンス領域で設定できます。デフォルトは有効です。作成後は、DBインスタンス一覧でDBインスタンスグループを選択し、**修正**をクリックして変更できます。

この設定はDBインスタンスグループ全体に同様に適用されます。Primary、Standby、Read Replicaがそれぞれ異なる設定を持つことはできません。Read Replicaを新たに作成した場合、グループの設定がそのまま適用されます。

**適用条件**

次の条件をすべて満たすDBインスタンスに適用されます。

| 条件                                                         | 満たさない場合         |
|--------------------------------------------------------------|----------------|
| ファミリーパラメータグループを使用                                          | 適用対象から除外されます。 |
| 現在のDBエンジンバージョンが自動アップグレード対象                               | 適用対象から除外されます。 |
| 予約または保留中のDBエンジンバージョンアップグレード作業なし                         | 次の周期に延期されます。  |
| PrimaryはすべてのRead Replicaが先にアップグレードされた状態                  | 次の周期に延期されます。  |

自動アップグレード対象バージョンは、同じメジャーバージョン内のマイナーバージョンに限定されます。メジャーバージョンアップグレードは自動的には実行されません。

ユーザーが予約した、またはProviderが登録したDBエンジンバージョンアップグレード作業が残っている場合、自動メンテナンス作業を登録せずに譲ります。残っていた作業が処理されると、次の周期に再度登録されます。

!!! tip "参考"
    単一パラメータグループを使用するDBインスタンスには適用されません。自動アップグレードを使用するには、[ファミリーパラメータグループ](parameter-group-gov/#family-parameter-group)に変更する必要があります。

**適用順序**

レプリケーション構成を維持するため、Read Replicaを先にアップグレードしてからPrimaryをアップグレードします。高可用性DBインスタンスのPrimaryとStandbyは、1つの作業として同時にアップグレードされます。この順序により、DBインスタンスグループ全体に反映されるまでに複数のメンテナンス期間がかかる場合があります。

**対象判定周期**

自動アップグレード対象かどうかは 1日に1回判定されます。対象の場合は、次回のメンテナンス期間に実行される自動メンテナンス作業が登録されます。

**作業除外**

登録された自動メンテナンス作業を今回のメンテナンス期間に実行したくない場合は、メンテナンスタブで**保留**を選択します。保留した作業は再度登録されません。**次回のメンテナンス期間に適用**を選択すると、その時点で実行されます。

自動アップグレード自体を中断するには、DBインスタンスグループを修正してマイナーバージョン自動アップグレードを無効に変更します。無効に変更すると、まだ実行されていない自動メンテナンス作業も合わせて削除されます。
...
準備中のメンテナンス作業は、選択後に**削除**または**保留**をクリックすることで、メンテナンス期間から除外できます。
削除されたユーザーメンテナンス作業はキャンセルされ、再度メンテナンス期間に適用するには、該当の作業を再実行する必要があります。
Providerメンテナンス作業と自動メンテナンス作業は、保留中のメンテナンス一覧に移動します。保留中のメンテナンス一覧から、再度準備中のメンテナンス作業に移動できます。

<a id="db-instance-details-maintenance-pending-maintenance"></a>
#### 保留中のメンテナンス

保留中のメンテナンスは、NHN Cloudが提供するProviderメンテナンス作業と、システムが自動的に登録した自動メンテナンス作業の一覧です。パラメータグループの変更の適用、ハイパーバイザー点検のためのマイグレーション、マイナーバージョン自動アップグレードなどの作業が含まれます。

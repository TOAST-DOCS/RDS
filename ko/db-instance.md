...
| WAIT_MANUAL_CONTROL      | 장애 조치 수동 제어 대기 중 |
...
DB 인스턴스는 HDD, SSD 2가지 데이터 스토리지 종류를 지원합니다. 데이터 스토리지 종류에 따라 성능과 가격이 다르므로, 데이터베이스 워크로드에 따라 알맞은 데이터 스토리지 종류를 선택해야 합니다. 데이터 스토리지는 최소 20GB~2TB까지 생성할 수 있습니다.

### 데이터 스토리지 크기 확장

DB 인스턴스의 데이터 스토리지 종류는 변경할 수 없지만 데이터 스토리지 크기는 웹 콘솔에서 손쉽게 확장할 수 있습니다. 데이터 스토리지 크기 확장 과정에서 DB 인스턴스가 종료되며, 서비스 부하에 따라 수분의 다운 타임이 발생합니다. 읽기 복제본이 존재하는 경우 마스터의 데이터 스토리지 크기 확장 시 읽기 복제본의 데이터 스토리지 크기도 같이 확장됩니다. 읽기 복제본이 여러 대 있는 경우 데이터 스토리지 크기 확장은 순차적으로 진행됩니다. 데이터 스토리지 크기 확장 중 오류가 발생할 경우 일부 읽기 복제본의 데이터 스토리지 크기가 확장되지 않을 수 있으며, 확장에 실패한 읽기 복제본의 경우 이후 개별적으로 확장 가능합니다. 데이터 스토리지 크기는 현재 크기보다 작게 변경할 수 없습니다.
...
### 사용자 & 스키마 직접 제어
RDS for MySQL 에서는 웹 콘솔에서 사용자와 스키마를 손쉽게 관리할 수 있는 기능을 제공하지만, 사용자가 직접 관리하고 싶은 경우에 직접 제어 기능을 제공합니다. 웹 콘솔의 DB 인스턴스 수정 화면에서 사용할 수 있습니다.
* 직접 제어를 사용하면 현재 생성되어 있는 모든 유저에게 해당 권한을 부여합니다.
```sql
GRANT CREATE,DROP,LOCK TABLES,REFERENCES,EVENT,ALTER,INDEX,INSERT,SELECT,UPDATE,DELETE,CREATE VIEW,
SHOW VIEW,CREATE ROUTINE,ALTER ROUTINE,EXECUTE,CREATE USER,PROCESS,RELOAD,REPLICATION SLAVE,REPLICATION CLIENT,
SHOW DATABASES, CREATE TEMPORARY TABLES,TRIGGER ON *.* TO '{user_id}'@'{host}' WITH GRANT OPTION;
```

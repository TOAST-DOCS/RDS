...
* 서버대시보드의 MySQL 지표중, Connection 차트에 Max Connection 값을 추가로 표기하도록 수정

#### 버그 수정

* IAM 콘솔에서 에러 발생 시, 적절한 에러 페이지로 이동하지 못하는 문제 수정
* 장애 조치를 이용한 DB인스턴스 타입 변경, 스토리지 확장을 수행 할 경우, Ping 간격이 기본값으로 설정되는 문제 수정

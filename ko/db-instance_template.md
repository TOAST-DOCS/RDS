...
![db-instance-list_ko]({{url.cdn}}/26.01.13/db-instance-list_ko.png)
...
![db-instance-filter_ko]({{url.cdn}}/26.01.13/db-instance-filter_ko.png)
...
![db-instance-detail_ko]({{url.cdn}}/26.01.13/db-instance-detail_ko.png)
...
![db-instance-detail-log_ko]({{url.cdn}}/26.01.13/db-instance-detail-log_ko.png)
...
![db-instance-detail-log-bin_ko]({{url.cdn}}/24.03.12/db-instance-detail-log-bin_ko.png)
...
![modify-ha-popup-ko]({{url.cdn}}/26.01.13/modify-ha-popup-ko.png)
...
![db-instance-os-upgrade-ko.png]({{url.cdn}}/26.01.13/db-instance-os-upgrade-ko.png)
...
![db-instance-os-upgrade-single-popup-ko.png]({{url.cdn}}/26.01.13/db-instance-os-upgrade-simple-popup-ko.png)
...
![os-upgrade-ha-popup-ko.png]({{url.cdn}}/26.01.13/os-upgrade-ha-popup-ko.png)
...
![db-instance-list-parameter-ko]({{url.cdn}}/26.01.13/db-instance-list-parameter-ko.png)
...
![db-instance-parameter-ha-ko]({{url.cdn}}/26.01.13/db-instance-parameter-ha-ko.png)
...
![db-instance-replica-create-ko]({{url.cdn}}/26.01.13/db-instance-replica-create-ko.png)
...
!!! danger "주의"
    리전 피어링이 연결되어 있더라도 라우트 설정이 올바르지 않을 경우 Read Replica 생성에 실패하거나 복제가 중단될 수 있습니다.
{{/if}}
...
{{engine.pascalCase}}을 재시작하거나 고가용성 DB 인스턴스를 수동으로 장애 조치하려면 DB 인스턴스를 재시작할 수 있습니다. 재시작 시간을 최소화하기 위해 서비스 부하가 낮은 시간대에 수행하기를 권장합니다. 고가용성 DB 인스턴스는 장애 조치를 이용한 재시작을 사용하지 않으면 Standby를 먼저 재시작한 뒤 Primary를 재시작합니다. 장애 조치 기능을 이용한 재시작은 [수동 장애 조치](#manual-failover) 항목을 참고합니다.

DB 인스턴스 재시작을 하려면 콘솔에서

![db-instance-restart-ko]({{url.cdn}}/26.01.13/db-instance-restart-ko.png)
...
![db-instance-restart-force-ko]({{url.cdn}}/26.01.13/db-instance-restart-force-ko.png)
...
![db-instance-deletion-protection-ko]({{url.cdn}}/26.01.13/db-instance-deletion-protection-ko.png)

❶ 삭제 보호 설정을 변경하려는 DB 인스턴스를 선택 후 드롭다운 메뉴에서 **삭제 보호 설정 변경** 메뉴를 클릭하면 팝업 창이 나타납니다.

![deletion-protection-popup-ko]({{url.cdn}}/24.03.12/deletion-protection-popup-ko.png)
...
!!! tip "알아두기"
    남은 시간은 지금까지의 평균 반영 속도로 계산한 추정값이므로 실제 소요 시간과 다를 수 있습니다.

### 장애 조치 되돌리기

Standby에 반영할 복제 로그가 많으면 복제 로그 반영 단계가 길어집니다. 이때는 장애 조치를 끝까지 기다리는 것보다 장애 조치를 되돌리고 장애가 발생한 Primary를 재시작하는 편이 더 빨리 서비스를 재개하는 방법일 수 있습니다. 진행 단계가 Failed Over Primary 차단 또는 복제 로그 반영일 때 DB 인스턴스 그룹 행의 이름 옆에 **장애 조치 되돌리기** 버튼이 나타나며, 버튼을 클릭하면 경고 팝업이 나타난 후 되돌리기가 실행됩니다.
...
![db-instance-failover-repair-ko]({{url.cdn}}/26.01.13/db-instance-failover-repair-ko.png)
...
![db-instance-failover-rebuild-ko]({{url.cdn}}/26.01.13/db-instance-failover-rebuild-ko.png)
...
![db-instance-failover-split-ko]({{url.cdn}}/26.01.13/db-instance-failover-split-ko.png)
...
![db-instance-ha-wait-manual-failover-ko]({{url.cdn}}/26.01.13/db-instance-ha-wait-manual-failover-ko.png)
...
!!! danger "주의"
    복제용 계정이 복제 대상(Master) {{engine.pascalCase}}에 생성되어 있어야 합니다.
...
예: {{engine.pascalCase}} error code 1236 (ER_MASTER_FATAL_ERROR_READING_BINLOG): Got fatal error from master when reading data from binary log
...
* 데이터를 내보낼 외부 {{engine.pascalCase}} 인스턴스에 연결합니다.
* 아래 명령어로 외부 {{engine.pascalCase}} 인스턴스에서 데이터를 백업합니다.
* 외부 {{engine.pascalCase}} 인스턴스(마스터)에서 가져올 경우
...
* 외부 {{engine.pascalCase}} 인스턴스(슬레이브)에서 가져올 경우
...
![patch-security-list-ko]({{url.cdn}}/26.05.12/patch-security-list-ko.png)

❶ 보안 패치 유지 관리 **보기** 버튼 클릭
❷ 현재 DB 이미지에 해당하는 보안 취약점 정보를 확인할 수 있습니다.

![patch-security-detail-ko]({{url.cdn}}/26.05.12/patch-security-detail-ko.png)

보안 패치를 적용하면 해결할 수 있는 보안 취약점 정보를 확인할 수 있습니다.

![patch-security-popup-ko]({{url.cdn}}/26.05.12/patch-security-popup-ko.png)
...
![patch-security-running-ko]({{url.cdn}}/26.05.12/patch-security-running-ko.png)

...
* 장애가 발생해 고가용성 인스턴스에 장애 조치가 수행됐을 때, 변경된 새 Master 인스턴스는 기존 Master 인스턴스의 백업을 승계하지만, 장애 조치 이전으로 시점복원은 불가능합니다.
...
* TOAST Cloud Compute & Network 상품에서 제공하는 일부 사양으로 DB 인스턴스를 생성할 수 있습니다.
...
* 백업이 실행되는 동안 데이터베이스 부하가 심하면 백업이 실패할 수 있습니다.
* 서비스에 영향을 주지 않기 위해서, 서비스의 부하가 적은 시간에 백업을 하거나, 읽기 복제본 또는 고가용성 구성을 사용하는 것을 권장합니다.

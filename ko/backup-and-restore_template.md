...
## Database > RDS for {{engine.pascalCase}} > 백업 및 복원 { #database-rds-for-enginepascalcase-backup-and-restoration }
...
![db-instance-backup-ko]({{url.cdn}}/24.03.12/db-instance-backup-ko.png)
![backup-list-1-ko]({{url.cdn}}/24.09.10/backup-list-1-ko.png)
...
![db-instance-backup-form-ko]({{url.cdn}}/24.11.12/db-instance-backup-form-ko.png)
...
![db-instance-list-export-obs-modal-ko]({{url.cdn}}/24.03.12/db-instance-list-export-obs-modal-ko.png)
...
![backup-export-ko]({{url.cdn}}/24.03.12/backup-export-ko.png)
...
백업을 이용하여 원하는 시점으로 데이터를 복원할 수 있습니다. 복원 시 항상 새로운 DB 인스턴스가 생성되며, 기존 DB 인스턴스에 복원할 수 없습니다. 백업을 수행한 원본 DB 인스턴스와 동일한 DB 엔진 버전으로만 복원할 수 있습니다. 백업이 생성된 시점으로 복원하는 스냅숏 복원, 원하는 특정 시점으로 복원하는 시점 복원을 지원합니다. RDS for {{engine.pascalCase}}에서 생성한 백업뿐만 아니라 외부 {{engine.pascalCase}}의 백업으로도 복원할 수 있습니다.
...
![db-instance-snapshot-restoration-ko]({{url.cdn}}/24.03.12/db-instance-snapshot-restoration-ko.png)

❶ DB 인스턴스의 상세 탭에서 복원할 백업 파일을 선택한 뒤 **스냅숏 복원**을 클릭하면 DB 인스턴스 복원 화면으로 이동합니다.

또는

![snapshot-restoration-ko]({{url.cdn}}/24.03.12/snapshot-restoration-ko.png)
...
* 바이너리 로그(binary log) 보관 기간에 따라 {{engine.pascalCase}}에 의해 자동으로 바이너리 로그(binary log)가 삭제된 경우
* 고가용성 DB 인스턴스의 장애 조치로 인해 바이너리 로그(binary log)가 삭제된 경우
* 기타 다양한 이유로 바이너리 로그(binary log)가 손상되거나 삭제된 경우

콘솔에서 시점 복원을 하려면

![point-in-time-restoration-list-ko]({{url.cdn}}/24.03.12/point-in-time-restoration-list-ko.png)
...
![point-in-time-restoration-01-ko]({{url.cdn}}/24.03.12/point-in-time-restoration-01-ko.png)

❶ 복원 방법을 선택합니다.

![point-in-time-restoration-02-ko]({{url.cdn}}/24.03.12/point-in-time-restoration-02-ko.png)

❷ 복원 시각을 선택합니다. 가장 최근 시점으로 복원하거나, 원하는 특정 시점을 직접 입력할 수 있습니다.

![point-in-time-restoration-03-ko]({{url.cdn}}/24.03.12/point-in-time-restoration-03-ko.png)
...
![point-in-time-restoration-04-ko]({{url.cdn}}/24.03.12/point-in-time-restoration-04-ko.png)
...
### 외부 {{engine.pascalCase}} 백업을 이용한 복원 { #restore-from-external }
...
!!! danger "주의"
    innodb_data_file_path의 설정값이 ibdata1:12M:autoextend가 아니면 RDS for {{engine.pascalCase}}의 DB 인스턴스로 복원할 수 없습니다.
...
### RDS for {{engine.pascalCase}} 백업을 이용한 복원 { #restoration-by-using-rds-for-enginepascalcase-backup }
...
(1) [백업 내보내기](backup-and-restore/#export) 항목을 참고하여 RDS for {{engine.pascalCase}}의 백업을 Object Storage로 내보냅니다.

(2) Object Storage의 백업을 복원하고자 하는 서버에 다운로드합니다.

(3) {{engine.pascalCase}} 서비스를 정지합니다.

(4) {{engine.pascalCase}} 데이터 저장 경로의 모든 파일을 삭제합니다.
...
(7) {{engine.pascalCase}} 서비스를 시작합니다.